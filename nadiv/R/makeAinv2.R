makeAinv2 <- function(pedigree, det = FALSE)
{
...
#  nA <- nA + 2 * sum(duplicated(paste(numped[, 2], numped[, 3])[bnmiss]) == FALSE)
  ...
  numped[numped == -998] <- N + 1
   ...
   Ainv <- as(tcrossprod(Ainv), "dgCMatrix")
...
library(nadiv)
load("~/Desktop/Dropbox/J_Poissant/large_gt_ped.Rdata")
peds <- prepPed(large.gt.ped[, c(1,3,2)])
peds$gen <- genAssign(peds)
pedin <- peds[which(peds$gen < 26), 1:3]
dim(pedin)

#system.time(Ainv <- makeAinv(pedin))
#system.time(Ainv2 <- makeAinv2(pedin))
system.time(A <- makeA(pedin))
